...
public class Vbox2
{
    ...
    static void testEnumeration(VirtualBoxManager mgr, IVirtualBox vbox)
    {
        ...
        for (IMachine m : machs)
        {
            ...
            if (!inaccessible)
            {
                ...
                System.out.println("VM descr: " + m.getId());
            }
        }
        // process system event queue
        mgr.waitForEvents(0);
    ...
    static void testStart(VirtualBoxManager mgr, IVirtualBox vbox) throws Exception
    {
        IMachine m = vbox.getMachines().get(0);
        System.out.println(m.getSessionState());
        //String name = m.getName();
        String name = "Mininet-VM";
        ...
        //IProgress pau = m.getConsole.pause(name);
       /* boolean pau = mgr.startVm(name, name, 1);
        ISession session = mgr.getSessionObject();
        session.unlockMachine();*/
        //System.out.println(machineVM.getSessionState());
        //IProgress progress = m.getConsole().powerDown();
        //IProgress progress = machineSessionVM.getConsole().powerDown();
        
        
        /*//APAGAR VM
        TimeUnit.SECONDS.sleep(30);
        VirtualBoxManager vboxManager = VirtualBoxManager.createInstance(null);
        vboxManager.connect("http://localhost:18083","user", "password");
        IMachine machineVM = vboxManager.getVBox().findMachine(name);
        ISession machineSessionVM = vboxManager.openMachineSession(machineVM);
        IProgress progress = machineSessionVM.getConsole().powerDown();*/
    }
    //APAGAR VM
    static void testStop(VirtualBoxManager mgr, IVirtualBox vbox) throws Exception
    {
        IMachine m = vbox.getMachines().get(0);
        //String name = m.getName();
        String name = "Mininet-VM";
        System.out.println("\nAttempting to Stop VM '" + name + "'");
        ...
        machineSessionVM.getConsole().pause();
        System.out.println(machineSessionVM.getState());
        System.out.println(machineSessionVM.getMachine().getAccessible());
        TimeUnit.SECONDS.sleep(10);
        
        //CORRER NUEVAMENTE LA VM
        System.out.println("corriendo nuevamente");
        machineSessionVM.getConsole().resume();
        
        System.out.println(machineSessionVM.getMachine().getAccessible());
        System.out.println(machineSessionVM.getMachine().getHardwareVersion());
        System.out.println(machineSessionVM.getMachine().getName());
        
        TimeUnit.SECONDS.sleep(10);
        
        System.out.println("APAGANDO");
        machineSessionVM.getConsole().powerDown();
        //IProgress progress = machineSessionVM.getConsole().powerDown();
        System.out.println(machineSessionVM.getMachine().getAccessible());
        System.out.println(machineSessionVM.getMachine().getHardwareVersion());
        System.out.println(machineSessionVM.getMachine().getName());
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        try
        {
            ...
            if (vbox != null)
            {
                ...
                testEnumeration(mgr, vbox);//Listar VMs
                testReadLog(mgr, vbox);
                testStart(mgr, vbox);//Iniciar VM
                TimeUnit.SECONDS.sleep(10);
                testStop(mgr, vbox);//Apagar VM
